...
boolean minmax = MAX; //Variavel para escolher se o algoritmo vai buscar mínimos ou máximos{ TRUE = MAX, FALSE = MIN}
...
int main(void){
    ...
    FILE* comandosnormais=NULL;
    FILE* comandosgnuplot=NULL; 
    ...
    comandosnormais=popen("export DISPLAY=localhost:0","w");
    fclose(comandosnormais);
    ...
    while(opcoes!='q'){ // O programa inicia com tudo já inicializado
        ...
        switch(mododereproducao){
            ...
            if(autoplot==TRUE){
                fflush(plotgeracoes);
                fflush(plotmedia);
                fflush(plotfitmelhor);

                fprintf(comandosgnuplot,"set title 'Melhor fitness e fitness medio'\nset key above\nset xlabel 'Numero de Geracoes'\nset ylabel 'Fitness [F(x)]'\n");
                fprintf(comandosgnuplot,"plot 'MediafitparaPlot.txt' title 'Mediafit' with lines, 'MelhorfitparaPlot.txt' title 'Fitdomelhor' with lines \n");
                fflush(comandosgnuplot);
            }
        ...
        if(plotgeracoes!=NULL)       fclose(plotgeracoes);
        if(plotmedia!=NULL)          fclose(plotmedia);
        if(plotfitmelhor!=NULL)      fclose(plotfitmelhor);
        if(plotmelhor!=NULL)         fclose(plotmelhor);
        if(comandosnormais!=NULL)    fclose(comandosnormais);
        if(comandosgnuplot!=NULL)    fclose(comandosgnuplot);
