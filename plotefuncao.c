...
double PORCENTAGEM_DE_MUTACAO = INICIAL_PORCENTAGEM_DE_MUTACAO ;
...
MEDIAEFIT busca_melhor(double* populacao,int tamanhodapopulacao); // Função que busca o melhor indivíduo na população (nesse caso o melhor é o que possui o maior valor na função F(x)=5-|x-42|)
...
double* inicia_populacao(int tamanhodapopulacao){
    ...
    for(i=0;i<tamanhodapopulacao;i++){ //Define os primeiros valores como aleatórios entre 0 e 1k com 2 casas decimais
        populacao[i]= (rand()%2000) + (double)((rand()%100)/(100.0f));
    ...
    */
    ...
    if(novotamanhodapop==TAMANHO_DA_POPULACAO) return populacao; // Nada acontece
        ...
        for(;i<novotamanhodapop;i++){ // Termina inserindo elementos aleatorios
            novapopulacao[i] = (rand()%2000) + (double)((rand()%100)/(100.0f));
        }
    }
        ...
        novapopulacao[0]=busca_melhor(populacao,TAMANHO_DA_POPULACAO).melhor;
...
MEDIAEFIT busca_melhor(double* populacao,int tamanhodapopulacao){
...
int main(void){
    ...
    int temp,geracao=1,contador=0,mutacoesdiferentes=0; // Variavel para armazeenar um inteiro temporarivamente, Contador de gerações, e contadores para mutacao adaptativa

    double antigomelhor; // Variáveis para armazenar o melhor individuo de uma população e compara-lo com o antigo melhor.
    ...
    while(saida!='q'){
        ...
        mediaefit=busca_melhor(populacao,TAMANHO_DA_POPULACAO); //Percorre toda a população buscando o melhor elemento (máximo da função F(x)=5-|x-42| )
        ...
        if(mutacaoadaptativa==TRUE){ /* Trecho de código referente a mutação adaptativa. Ele deve:
            ...
            if(antigomelhor!=mediaefit.melhor){ // Se o melhor mudou, quer dizer que melhorou (existe um novo melhor de todos), então reseta os contadores. Se não melhorou, o contador é incrementado
        ...
        switch(mododereproducao){
            ...
            case ELITISMO: // O melhor tem filhos com todos os outros 
                    ...
                    elitismo(populacao,mediaefit.melhor,TAMANHO_DA_POPULACAO); // O melhor é cruzado com todos os outros (soma e divide por 2 + uma mutação definida pelos defines), gerando uma nova população
            ...
            //Ele geram subpopulações ao redor de alguns ótimos --> Converge mais devagar porém não perde tanto a diversidade.
                    ...
                    torneio_de_dois(populacao,mediaefit.melhor,TAMANHO_DA_POPULACAO);
